...
#include "resource.h"
#include <atlframe.h>
class CTweetWindow : public ui_element_instance, public CWindowImpl<CTweetWindow>  {
public:
	DECLARE_WND_CLASS_EX(TEXT("now plyaingô"), CS_VREDRAW | CS_HREDRAW, (-1));

	void initialize_window(HWND parent) { WIN32_OP(Create(parent, 0, 0, 0, WS_EX_STATICEDGE) != NULL); }

	BEGIN_MSG_MAP(CTweetWindow)
	...
	CTweetWindow(ui_element_config::ptr, ui_element_instance_callback_ptr p_callback);
	...
	static void g_get_name(pfc::string_base & out) { out = "test"; }
	static ui_element_config::ptr g_get_default_configuration() { return ui_element_config::g_create_empty(g_get_guid()); }
	static const char * g_get_description() { return "‚Ù‚Ù‚Ù‚¢"; }
...
void CTweetWindow::notify(const GUID & p_what, t_size p_param1, const void * p_param2, t_size p_param2size) {
	if (p_what == ui_element_notify_colors_changed || p_what == ui_element_notify_font_changed) {
		// we use global colors and fonts - trigger a repaint whenever these change.
		Invalidate();
	}
}
CTweetWindow::CTweetWindow(ui_element_config::ptr config, ui_element_instance_callback_ptr p_callback) : m_callback(p_callback), m_config(config) {
}

BOOL CTweetWindow::OnEraseBkgnd(CDCHandle dc) {
	...
	WIN32_OP_D(dc.FillRect(&rc, brush));	return TRUE;
}
void CTweetWindow::OnPaint(CDCHandle) {
	CPaintDC dc(*this);
...
class ui_element_myimpl : public ui_element_impl_withpopup<CTweetWindow> {};
